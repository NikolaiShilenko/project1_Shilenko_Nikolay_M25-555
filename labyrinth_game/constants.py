...
ROOMS = {
    ...
    'orangery': {
        'description': 'Оранжерея с экзотическими растениями. В центре растет золотое дерево рядом с иссохшим источником, вокруг цветут редкие цветы.',
        'exits': {'east': 'armory'},
        'items': ['golden_tree', 'dried_spring', 'exotic_flowers'],
        'puzzle': ('Дерево шепчет: "В руках не удержать, в решете не унести. Что это?" (ответ одно слово)', 'вода')
    },
    'observatory': {
        'description': 'Обсерватория с телескопом. На столе лежат звездные карты.',
        'exits': {'west': 'treasure_room'},
        'items': ['star_map', 'telescope'],
        'puzzle': ('Карта показывает последовательность: 2, 4, 6, ? Какое следующее число? (ответ цифрой)', '8')
    }
}
